...
interface GameState {
  ...
  setCurrentFloor: (floor: 1 | 2 | 3) => void;
...
export const useGameStore = create<GameState>((set, get) => ({
  ...
  attackWithLantern: () => {
    ...
    if (!hasCombatItem || state.isDead || state.workshopOpen || state.socialPostOpen || state.gamePhase !== 'playing') return;
    ...
    if (weapon === 'blaster') sounds.playGunshot();
    else sounds.playBambooPoke();
  ...
  openSocialPost: () => {
    ...
    if (firstView) {
      get().showAchievement({
        ...
        subtitle: 'Up ảnh/meme vào carousel hoặc takeover màn hình bằng ảnh của bạn.',
      });
    }
  },
